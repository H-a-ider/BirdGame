...
def welcomeScreen():
    ...
    messagex = int((SCREENHEIGHT - GAME_SPRITES['message'].get_width())/5)
...
def mainGame():
    ...
    upperPipes = [
        ...
        {'x': SCREENWIDTH+200 + (SCREENWIDTH/200), 'y':newPipe2[0]['y']}
    ]

    # My list of lower pipes
    lowerPipes = [
        {'x': SCREENWIDTH+200, 'y':newPipe1[1]['y']},
        {'x': SCREENWIDTH+200 + (SCREENWIDTH/200), 'y':newPipe2[1]['y']}
    ]
...
def getRandomPipe():
    """
    Generate positions of two pipes(one bottom straight and one toprotated ) for blitting on the screen
    """
    pipeHeight = GAME_SPRITES ['pipe'][0].get_height()
...
if __name__ == "__main__":
    # This is will be the main point where our game start
    pygame.init() # Initialize all pygame's modules
    FPSCLOCK = pygame.time.Clock()
    pygame.display.set_caption("Flappy Bird by Zeeshan Haider")
    ...
    while True:
        welcomeScreen() # Shows welcome screen to the user until the user presses a button
